...
- **Android 应用** — 原生 APK，在手机上直接运行完整的词典服务。可直接在手机上导入 MDX 文件，应用会自动构建 SQLite 缓存，后续启动极快。
...
从 [Releases 页面](https://github.com/ChaosNyaruko/ondict/releases/latest) 下载最新 APK，安装到手机即可。

> **要求**：Android 11+（目标 API 36）。若从应用商店外安装，需在手机设置中开启"允许安装未知来源应用"。

### 工作原理

Android 应用通过 [gomobile](https://pkg.go.dev/golang.org/x/mobile/cmd/gomobile) 将完整的 Go 词典服务器内嵌到 APK 中。启动时在本地 `127.0.0.1:1345` 启动 HTTP 服务，并用 WebView 展示与桌面端完全相同的 Web 界面。

### 词典配置

1. 打开应用，点击 **Import Dict**（或菜单按钮）
2. 从本地存储选择 `.mdx` 文件
3. 设置词典类型（如 `LONGMAN/Easy`、`MDX` 等）
4. 重启应用 — 首次加载新词典时，应用会在后台构建 SQLite 缓存；之后每次启动从 SQLite 加载，启动时间不到一秒

### 数据存储

所有用户数据（导入的词典、生词本、查询历史）均存储在应用私有目录中，**更新 APK 不会丢失数据**，只有卸载应用才会清除数据。
